...
int main (int argc, char *argv[])
{
	...
	for (;;){
		...
		while(terminar == 0)
		{
			...
			{
				sprintf (respuesta,"%d",DameJugadorRapido(circuito));
				write (sock_conn, respuesta, strlen(respuesta));
			}
			printf ("Respuesta: %s\n", respuesta);
			//lo enviamos
...
void DameTiempoJugador (char id[20])
{
	MYSQL *conn;
	int err;
...
void DameCircuitoFav(char NOMBRE[20])
{
	MYSQL *conn;
	int err;
	// Estructura especial para almacenar resultados de consultas 
...
//Dame el jugador m�s rapido de un circuito
void DameJugadorRapido(char circuito[20])
{
	MYSQL *conn;
	int err;
	// Estructura especial para almacenar resultados de consultas 
	...
	//consulta SQL
	strcpy (consulta, "SELECT JUGADORMASRAPIDO FROM ListaDeCarrerines WHERE ListaDeCarrerines.CIRCUITO = '");
	...
	if(row==NULL)
		...
		strcpy(rapido, row[0]);
		while(row!=NULL){
		printf("El jugador m�s r�pido es: %s\n", rapido);
	}
		mysql_close (conn);
	}}}}}
